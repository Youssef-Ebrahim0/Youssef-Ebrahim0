...
* 👁️ Computer Vision
* ⚙️ AI Backend & APIs
* 🚀 Model Deployment & AI Engineering

I enjoy working across the entire AI lifecycle — from **data and model development to retrieval, evaluation, APIs, and deployment**.
...
`Scikit-learn` • `XGBoost` • `CNNs` • `ResNet` • `Hugging Face`

### 🧠 LLM / NLP

`LangChain` • `RAG` • `FAISS` • `BM25` • `Cross-Encoder` • `Embeddings`
...
`FastAPI` • `Django` • `Flask` • `Streamlit` • `Docker` • `REST APIs`

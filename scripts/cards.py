...
These are files in your own repo, so they render as long as GitHub renders.
    ...
    python scripts/cards.py --user Youssef-Ebrahim0 --out assets
...
def main(argv=None):
    ...
    for entry in wanted:
        ...
        owner = entry.get("owner", args.user)
        ...
        if owner.lower() == args.user.lower():
            ...
            try:
                ...
                print(
                    f"  !! {owner}/{repo_name} unavailable (HTTP {e.code}), skipped")
                continue

        if not src:
            print(f"  !! {owner}/{repo_name} not found, skipped")
            continue
